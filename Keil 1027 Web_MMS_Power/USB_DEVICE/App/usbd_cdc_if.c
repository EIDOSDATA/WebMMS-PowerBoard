...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
	...
	if (Buf[0] == 'k' || Buf[0] == 'K') // View Voltage
	{
		...
		sprintf(buf, "bat : %d%d.%d :: cap : %d%d.%d\r\n", btmp / 100, (btmp % 100) / 10, btmp % 10, ctmp / 100, (ctmp % 100) / 10, ctmp % 10);
		//sprintf(buf, "%d\r\n", tmp);
