...
static inline void Port_ReFresh() // void Port_ReFresh() __attribute__((always_inline))
{
	// ON : 1, OFF : 07
...
static inline void ResPonse_PSTAT(int p) // void ResPonse_PSTAT(int p) __attribute__((always_inline))
{
	Port_ReFresh();
	switch (p)
	{
	...
	default:
		sprintf(buf, "\r\n#pstat,1,%c\r\n"
				...
				"#pstat,4,%c\r\n"
				"#pstat,5,%c\r\n"
				"#pstat,6,%c\r\n\r\n", Action_Flag_S[0], Action_Flag_S[1],
				Action_Flag_S[2], Action_Flag_S[3], Action_Flag_S[4],
				Action_Flag_S[5]);
		break;
	}
}
...
static int8_t CDC_Receive_FS(uint8_t *Buf, uint32_t *Len)
{
	...
	if (Buf[0] == 'k' || Buf[0] == 'K')
	{
		sprintf(buf, "#vin,%d.%d\r\n"
				"#vcam,%d.%d\r\n\r\n", (int) voltage_bat,
				(int) (voltage_bat * 100) % 100, (int) voltage_cam,
				(int) (voltage_cam * 100) % 100);
